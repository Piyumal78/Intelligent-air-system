...
export const SystemProvider = ({ children }) => {
  ...
  const createUser = async (userData) => {
    ...
    try {
      ...
      console.log("[v0] User created successfully:", user.uid)
  ...
  const deleteUser = async (userId) => {
    try {
      // Note: Deleting from Auth requires Admin SDK or Cloud Function in production
      // Here we just delete from Firestore for the UI
  ...
  const registerDevice = async (deviceData) => {
    try {
      const deviceRef = doc(db, "devices", deviceData.id || deviceData.deviceId)
